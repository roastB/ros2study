...
class OrderBufferChecker(Node):
    ...
    def maker_status_callback(self, msg: Bool):
        ...
        if msg.data:
            ...
            order_msg.id = 1
            order_msg.ingredients = ['불고기샌드위치', '칠리', '양상추', '모짜렐라']  # 예: ['불고기샌드위치', '칠리', '양상추', '모짜렐라']
    ...
    def start_delivery_monitoring(self, selected_pinky):
        ...
        self.get_logger().info(f'   -> Pinky 서빙위치 도착 대기 중... (기준: {self.SERVING_POSITION_THRESHOLD}m 이하)')

    # 수정됨: CompleteOrder 타입으로 샌드위치 제조 완료 처리
    def sandwich_ready_callback(self, msg: CompleteOrder):
        """샌드위치 제조 완료 구독 콜백 (CompleteOrder 타입)"""
        if not self.is_monitoring_delivery_status:
            return

        self.is_sandwich_ready = True
        self.current_complete_order = msg  # 메시지 저장
        self.get_logger().info(f'[🥪] 샌드위치 제조 완료! → 주문 ID: {msg.id}')
        self.check_delivery_ready()  # push_sandwich 호출 제거

    def push_sandwich(self, msg: CompleteOrder):
        """샌드위치 제조 완료 → 서빙 준비 단계로 메시지 전달"""
        self.get_logger().info(f'[📤] 서빙 준비를 위해 main_handle로 푸시 → 주문 ID: {msg.id}')
    ...
    def maker_handle_callback(self, msg: CompleteOrder):
        ...
        if self.is_waiting_for_push_complete:
            self.get_logger().info(f'[✅] 샌드위치 밀기 완료! → 주문 ID: {msg.id}')
            self.is_waiting_for_push_complete = False  # 대기 상태 해제
            ...
            self.get_logger().info(f'[📥] maker_handle 메시지 수신 (대기 중 아님) → 주문 ID: {msg.id}')
    ...
    def check_delivery_ready(self):
        ...
        if self.is_sandwich_ready and self.is_pinky_at_serving_position:
            pinky_name = self.get_pinky_name_from_status(self.selected_pinky_for_delivery)
            self.get_logger().info(f'[✅] 배송 준비 완료! Pinky {pinky_name} + 샌드위치 준비됨')
            
            # 1단계: main_handle로 샌드위치 밀기 명령 발행
            if hasattr(self, 'current_complete_order'):
                self.push_sandwich(self.current_complete_order)
                self.is_waiting_for_push_complete = True  # 밀기 완료 대기 상태 설정
                self.get_logger().info('[⏳] 샌드위치 밀기 완료 응답 대기 중...')
            
            # 2단계는 maker_handle_callback에서 밀기 완료 후 실행
    ...
    def send_table_delivery_command(self, table_number):
        ...
        if pinky_command_value is not None:
            ...
            pinky_name = self.get_pinky_name_from_status(self.selected_pinky_for_delivery)
            self.get_logger().info(f'[✅] Pinky {pinky_name}에게 테이블 {table_number} "서빙 명령 전송"')
            self.get_logger().info(f'   -> pinky_command: {pinky_command_value} (Pinky {pinky_name} 지정)')
