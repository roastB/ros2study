...
class OrderReturn(Node):
    def __init__(self):
        ...
        self.return_timer = None  # 타이머 참조 저장
        self.ignore_next_command = False  # 자신이 보낸 복귀 명령 무시용
    ...
    def pinky_command_callback(self, msg):
        # 0은 복귀 명령으로 무한 루프 방지를 위해 처리하지 않음
        if msg.data == 0:
            self.get_logger().info('[-] pinky_command=0 received (return), ignoring to prevent loop')
            return

        # 자신이 보낸 복귀 명령 무시
        if self.ignore_next_command:
            self.get_logger().info(f'[-] Ignoring self-published pinky_command: {msg.data}')
            self.ignore_next_command = False
            return
    ...
    def pinky_task_callback(self, msg):
        if msg.data == 0:
            self.get_logger().info('[-] pinky_task=0 received (return), resetting flags for next serving')
            self.reset_flags()
            self.return_sent = False
            self.get_logger().info('[...] 다음 서빙 시작 대기중')
            return
    ...
    def check_return_condition(self):
        if (self.pickup_detect_by_sensor and self.pickup_detect_by_uibutton and not self.return_sent and self.latest_command is not None):
            ...
            # 5초 후에 복귀 명령 보내기 위한 타이머 시작
            self.get_logger().info('[✅] Pickup confirmed. Serving Complete!')
            self.saved_command = self.latest_command
            ...
            # 새 타이머 생성 (일회성)
            self.return_timer = self.create_timer(5.0, self.send_return_command_once)

            self.return_sent = True
            self.reset_flags()

    def send_return_command_once(self):
        # 다음에 오는 자신의 명령은 무시하도록 설정
        self.ignore_next_command = True
        
        # 복귀 명령 publish
        self.command_pub.publish(Int32(data=self.saved_command))
        self.get_logger().info(f'   -> Published pinky_command: {self.saved_command}')

        self.pinky_task_pub.publish(Int32(data=0))
        self.get_logger().info('   -> Published pinky_task: 0 (return)')
        
        # 타이머 취소해서 한 번만 실행되도록 함
        if self.return_timer is not None:
            self.return_timer.cancel()
            self.return_timer = None
            self.get_logger().info('[↻] Return timer cancelled - one-time execution complete')
    ...
    def reset_flags(self):
        ...
        # 타이머는 여기서 취소하지 않음 (복귀 명령 실행을 위해 유지)
